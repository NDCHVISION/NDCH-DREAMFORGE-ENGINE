...
const ANCHOR_FILENAME = 'NDCH_017_S1_anchor_v9.jpg';

// ── Anchor v9 prompt (~726 chars — within 1000-char Runway hard limit) ────────
const ANCHOR_PROMPT = `Deep black background. Two machined dark grey steel blocks dominate the frame. The left block fills the left half of the frame — its left edge exits the frame, its right inner face is the right boundary of the left zone. The right block fills the right portion — its right edge exits the frame, its left inner face is the left boundary of the right zone. Between the two inner faces: a dark wedge shape — triangular, with apex at the bottom center of the frame and wide open at the top. The apex — where the two faces come closest — is the focal point. A thin horizontal gold line #C6A94F spans the gap at the apex, connecting both inner faces at their point of nearest approach. Smooth matte surfaces. Hard square edges. 9:16 portrait.`;
  ...
  if (listRes.ok) {
    ...
    for (const asset of assets) {
      if (asset.name === filename) {
        console.log(`  [upload] deleting existing: ${asset.name} (id=${asset.id})`);
